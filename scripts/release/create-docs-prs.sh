...
maps_ios_upload_docs() {
    ...
    git -C "$staging_docs_path" reset --hard origin/publisher-production
    ...
    # git worktree remove "$staging_docs_path" --force
    popd
}
...
[Staging version](https://docs.tilestream.net/ios/maps/api/$VERSION/)"
    ...
    PRODUCTION_DOCS_PR_URL=$(GITHUB_TOKEN=$(mbx-ci github writer public token) \
        gh pr create --repo mapbox/mapbox-maps-ios \
            ...
            --body "$body" \
            --label "docs :scroll:")
        # --reviewer "@mapbox/maps-ios" # For some reason, gh says that team cannot be found. Reproducable with mbx-ci tokens
        # Let's make a separate API call to fix it

    PRODUCTION_DOCS_PR_NUMBER=${PRODUCTION_DOCS_PR_URL##*/}
    jq -n '{"team_reviewers":["maps-ios"]}' | gh api "/repos/mapbox/mapbox-maps-ios/pulls/$PRODUCTION_DOCS_PR_NUMBER/requested_reviewers" --silent --input -
...
ios_sdk_update_versions() {
    ...
    git clone "https://x-access-token:$(github_writer_token)@github.com/mapbox/ios-sdk.git" "$IOS_SDK_REPO_PATH" --branch publisher-production --depth=1 --quiet
...
ios_sdk_open_pr() {
    ...
    iOS_DOCS_PR_NUMBER=${iOS_DOCS_PR_URL##*/}
    jq -n '{"team_reviewers":["maps-ios"]}' | gh api "/repos/mapbox/ios-sdk/pulls/$iOS_DOCS_PR_NUMBER/requested_reviewers" --silent --input -
